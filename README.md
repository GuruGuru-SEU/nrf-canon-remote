...
图纸逐器件标注 `JLC C…` 物料编号，可在嘉立创EDA直接搜索；87 个元件对应 28 种立创物料。未核实或待选型的器件、DNP 和 PCB 自带焊盘分别注明。物料编号与完整 BOM 一致；2026-09-19 新核实的四项供应商链接见 BOM，采购状态仍为待采购。
...
检查覆盖 102 个 BOM 位号/焊盘位置、92 件默认装配物料、65 件电阻/电容/电感的数值、49 个 MCU 引脚、9 路按键 RC，以及图中全部 272 个端子的实际连线。浏览器检查包含文字重叠/裁切、移动页面、切页、缩放和位号定位；这些属于文档与绘图一致性检查，不能替代电气 ERC 和样机测试。网页可离线浏览，SVG 下载依赖同目录的 `schematic-assets/`。
